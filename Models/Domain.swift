...
extension Profile {
    var asDictionary: [String: Any] {
        return [
            "nickname": nickname,
            "yearOfBirthRange": yearOfBirthRange,
            "email": email,
    ...
    static func fromDictionary(_ dict: [String: Any]) -> Profile {
        let nickname = dict["nickname"] as? String ?? ""
        let yearOfBirthRange = dict["yearOfBirthRange"] as? String ?? ""
        let email = dict["email"] as? String ?? ""
        let phoneNumber = dict["phoneNumber"] as? String ?? ""
        let skinTypeRaw = dict["skinType"] as? String ?? SkinType.normal.rawValue
        let skinType = SkinType(rawValue: skinTypeRaw) ?? .normal
        let allergies = dict["allergies"] as? [String] ?? []
        let goalsRaw = dict["goals"] as? [String] ?? []
        let goals = goalsRaw.compactMap { SkinGoal(rawValue: $0) }
        let profileIcon = dict["profileIcon"] as? String ?? "person.fill"
